...
import { EXERCISE_TABS, type ExerciseTabKey } from "./exercise-tabs";

/** Same URL-driven tab strip as the group detail and create-problem pages. */
export function ExerciseTabNav({ active }: { active: ExerciseTabKey }) {
  return (
    <div className="scrollbar-none mb-5 overflow-x-auto border-b border-border">
      <nav className="flex min-w-max gap-1" aria-label="Mục của trang bài tập">
        {EXERCISE_TABS.map((tab) => {
          const isActive = tab.key === active;
          return (
            <Link
              key={tab.key}
              href={`/exercises?tab=${tab.key}`}
