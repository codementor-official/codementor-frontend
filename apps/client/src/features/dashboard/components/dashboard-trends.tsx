...
export function DashboardTrends({ data, onRetry }: { data: LearningDashboard | null; onRetry: () => void }) {
  ...
  return <Card className="p-5">
    <div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="flex items-center gap-2 text-base font-bold text-navy"><TrendingUp className="h-4 w-4 text-primary" /> Nhịp học của bạn</h2><p className="mt-1 text-xs text-text-muted">Hoạt động gồm ghi danh, hoàn thành khóa/bài học và lần đầu giải đạt bài công khai.</p></div><div className="flex gap-1">{[7, 28].map((value) => <Button key={value} size="sm" variant={range === value ? 'primary' : 'outline'} aria-pressed={range === value} onClick={() => setRange(value)}>{value} ngày</Button>)}</div></div>
    <div className="my-4 grid divide-y divide-border-soft rounded-lg border border-border sm:grid-cols-3 sm:divide-x sm:divide-y-0"><div className="p-3"><strong className="text-xl font-bold tabular-nums text-navy">{total}</strong><span className="ml-2 text-sm text-text-muted">hoạt động</span></div><p className="p-3 text-xs text-text-muted">{previous ? `${total >= previous ? '+' : ''}${total - previous} so với ${range} ngày trước` : 'Giai đoạn trước chưa có hoạt động'}</p><p className="flex items-center gap-1.5 p-3 text-xs text-text-muted"><CalendarDays className="h-4 w-4" />{active}/{range} ngày có hoạt động</p></div>
    <div className="h-40 min-w-0" aria-label={`Biểu đồ ${total} hoạt động trong ${range} ngày`}>
