...
export function DashboardTrends({ data, onRetry }: { data: LearningDashboard | null; onRetry: () => void }) {
  ...
  return <Card className="p-5">
    ...
    <div className="my-5 flex flex-wrap items-end gap-x-6 gap-y-2"><div><strong className="text-3xl font-bold tabular-nums text-navy">{total}</strong><span className="ml-2 text-sm text-text-muted">hoạt động</span></div><p className="text-xs text-text-muted">{previous ? `${total >= previous ? '+' : ''}${total - previous} so với ${range} ngày trước` : 'Giai đoạn trước chưa có hoạt động'}</p><p className="ml-auto flex items-center gap-1.5 text-xs text-text-muted"><CalendarDays className="h-4 w-4" />{active}/{range} ngày tích cực</p></div>
