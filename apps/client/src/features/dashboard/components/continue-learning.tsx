...
export function ContinueLearning({ data, onRetry, excludeHref }: { data: LearningDashboard | null; onRetry: () => void; excludeHref?: string }) {
  const formatActivity = (date: string) => new Intl.DateTimeFormat('vi-VN', { day: '2-digit', month: '2-digit', timeZone: data?.timezone }).format(new Date(date));
  const items = [
  ...
  return <DashboardSection className="flex h-full flex-col" title="Tiếp tục học" href="/courses" description="Nội dung bạn học gần đây nhất, cùng tiến độ đã lưu.">
    {unavailable && <DashboardUnavailable className="flex-1" onRetry={onRetry} />}
    {!items.length && !unavailable && <DashboardEmpty className="flex-1" text="Bắt đầu một khóa học hoặc lộ trình để lưu tiến độ của bạn tại đây." href="/courses" action="Tìm khóa học đầu tiên" />}
    {items.length > 0 && <Card className="flex-1 overflow-hidden"><div className="hidden grid-cols-[minmax(0,1fr)_minmax(140px,200px)_90px] gap-3 border-b border-border-soft bg-bg px-4 py-2.5 text-2xs font-semibold text-text-muted sm:grid"><span>Nội dung</span><span>Tiến độ</span><span>Gần nhất</span></div><ul className="divide-y divide-border-soft">{items.map((item) => <li key={item.id}><Link href={item.href} className="grid items-center gap-3 p-4 transition-colors duration-150 hover:bg-bg sm:grid-cols-[minmax(0,1fr)_minmax(140px,200px)_90px]"><div className="flex min-w-0 items-center gap-3"><span className="rounded-lg border border-border bg-bg p-2 text-primary"><item.icon className="h-4 w-4" /></span><div className="min-w-0"><p className="truncate text-sm font-semibold text-navy">{item.title}</p><p className="mt-0.5 text-xs text-text-muted">{item.meta}{item.paused ? ' · Tạm dừng' : ''}</p></div></div><ProgressBar value={item.progress} label={`${Math.round(item.progress)}% hoàn thành`} /><span className="text-xs font-semibold text-primary">{formatActivity(item.date)} →</span></Link></li>)}</ul></Card>}
  </DashboardSection>;
}
