...
export function DashboardRecommendations({ excludeHrefs = [] }: { excludeHrefs?: string[] }) {
  ...
  const itemHref = (id: string) => `${kind === 'exercises' ? '/solve' : selected.href}/${id}`;
  const visibleItems = data?.items.filter((item) => !excludeHrefs.includes(itemHref(item.id))).slice(0, 3) ?? [];
