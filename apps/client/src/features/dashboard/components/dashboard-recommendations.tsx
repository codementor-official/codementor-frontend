...
export function DashboardRecommendations({ viewMode }: { viewMode: DashboardViewMode }) {
  ...
  return <DashboardSection title="Gợi ý bước học tiếp theo" href={selected.href}>
    ...
    {isLoading ? <Card className="h-56 animate-pulse bg-border-soft" aria-label="Đang tải đề xuất" /> : error ? <DashboardUnavailable onRetry={reload} /> : data && <>
      ...
      {!data.items.length ? <DashboardEmpty text="Chưa có nội dung phù hợp để đề xuất. Bạn có thể khám phá danh mục hoặc cập nhật sở thích." href={selected.href} action="Khám phá danh mục" /> : viewMode === 'table' ? <Card className="overflow-hidden"><ul className="divide-y divide-border-soft">{data.items.map((item) => <li key={item.id}><Link href={`${kind === 'exercises' ? '/solve' : selected.href}/${item.id}`} className="grid items-center gap-2 p-4 transition-colors hover:bg-bg sm:grid-cols-[110px_minmax(0,1fr)_minmax(160px,0.8fr)_auto]"><span className="text-2xs font-semibold text-primary">{selected.label}</span><span className="truncate text-sm font-semibold text-navy">{item.title}</span><span className="truncate text-xs text-text-muted">{item.reasons[0] ?? 'Nội dung công khai phù hợp.'}</span><span className="text-xs font-semibold text-primary">Mở →</span></Link></li>)}</ul></Card> : <ul className="grid gap-3 sm:grid-cols-2">{data.items.map((item) => <li key={item.id}><Link href={`${kind === 'exercises' ? '/solve' : selected.href}/${item.id}`} className="block h-full"><Card className="h-full p-4 transition-colors duration-150 hover:border-primary/50">
