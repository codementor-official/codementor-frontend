...
import { LoginError, setSession, signInWithPassword } from "@/features/auth/server/session";
...
export async function POST(request: NextRequest) {
  ...
  if (recent.length > 10) return NextResponse.json({ message: "Thử quá nhiều lần. Vui lòng đợi một phút." }, { status: 429 });
  const body = (await request.json().catch(() => null)) as { username?: unknown; password?: unknown } | null;
  const username = typeof body?.username === "string" ? body.username.trim() : "";
  const password = typeof body?.password === "string" ? body.password : "";
  if (!username || !password) return NextResponse.json({ message: "Vui lòng nhập email và mật khẩu." }, { status: 400 });
  try {
    ...
    return NextResponse.json({ message: error instanceof LoginError ? error.message : "Không thể xác minh đăng nhập." }, { status });
  }
}
