...
import { api, type ContentKind, type ModerationDecision, type QueueItem } from "@/lib/api";
...
export default function ModerationPage() {
  ...
  const load = useCallback(async () => {
    ...
    const results = await Promise.allSettled(wanted.map((each) => api.queue(each)));
  ...
  const decide = async (item: QueueItem, decision: ModerationDecision) => {
    ...
    setError(null);
    try {
      await api.moderate(item.kind!, item.id, decision, reason.trim() || undefined);
      setReason("");
      await load();
    } catch (cause) {
