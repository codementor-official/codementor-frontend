...
function userFromPayload(payload: JWTPayload): User {
  ...
  if (!payload.sub || !username) throw new Error("Keycloak token is missing user identity claims");
  const realmAccess = payload.realm_access as { roles?: unknown } | undefined;
  const allowedRoles = new Set<Role>(["STUDENT", "LECTURER", "ADMIN", "AI_AGENT"]);
  const roles = Array.isArray(realmAccess?.roles)
    ? realmAccess.roles.filter((role): role is Role =>
