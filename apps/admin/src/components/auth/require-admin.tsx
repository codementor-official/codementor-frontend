"use client";

import { useEffect } from "react";
import { useRouter } from "next/navigation";
import { ShieldAlert } from "lucide-react";
import { hasRole } from "@codementor/auth";
import { Button } from "@codementor/ui";
import { useAuth } from "@/providers/auth-provider";

/**
 * Tiện lợi, không phải hàng rào. Hàng rào thật là backend: mọi endpoint kiểm duyệt đều
 * có `@Roles('admin')` và use case kiểm lại vai trò. Ở đây chỉ để người không có quyền
 * khỏi nhìn một màn hình toàn 403.
 */
export function RequireAdmin({ children }: Readonly<{ children: React.ReactNode }>) {
  const { status, user, signOut } = useAuth();
  const router = useRouter();

  useEffect(() => {
    if (status === "anonymous") router.replace("/login");
  }, [status, router]);

  if (status !== "authenticated") {
    return (
      <main className="flex min-h-screen items-center justify-center bg-background">
        <p className="text-sm text-muted-foreground">Đang tải…</p>
      </main>
    );
  }

  if (!hasRole(user, "admin")) {
    return (
      <main className="flex min-h-screen items-center justify-center bg-background p-6">
        <div className="w-full max-w-md">
          <ShieldAlert aria-hidden="true" className="size-6 text-muted-foreground" />
          <h1 className="mt-3 text-lg font-semibold">Trang này chỉ dành cho quản trị viên</h1>
          <p className="mt-1.5 text-sm text-muted-foreground">
            Tài khoản <span className="font-medium text-foreground">{user?.email}</span> đang ở
            vai trò <span className="font-medium text-foreground">{user?.role}</span>.
          </p>
          <Button className="mt-5" onClick={signOut} type="button" variant="outline">
            Đăng xuất
          </Button>
        </div>
      </main>
    );
  }

  return <>{children}</>;
}
