...
export function Tab({ pane, kind }: { pane: PaneId; kind: TabKind }) {
  const { panes, setActive, closeTab, dragging, setDragging, setDropTarget, setContentDropTarget } = useWorkspace();
  ...
  return (
    <button
      ...
      className={`group/tab relative flex shrink-0 cursor-pointer items-center gap-1.5 rounded-none border-r border-b-2 border-border-soft px-3 py-2 text-xs font-medium whitespace-nowrap transition-colors duration-150 active:cursor-grabbing ${
        active
          ? "border-b-navy text-navy"
          : "border-b-transparent text-text-faint hover:border-b-border hover:text-navy"
